...
<div class="col-md-12">
    ...
    <div class="card">
        <div class="card-header card-header-icon card-header-primary">
            ...
            <h3 class="card-title ">
                <span id="table_name">ตารางข้อมูลพื้นฐานรายรับ - รายจ่าย</span>
            </h3>
    ...
    <div class="modal-dialog">
        <div class="modal-content" style="width:700px">
            <div class="modal-header">
                ...
                <!--  ปุ่มปิด -->
                <button class="btn acr_button" id="btn_close" rel="tooltip" data-dismiss="modal" title="" data-original-title="คลิกเพื่อปิด">ปิด</button>
            </div>
...
function get_data() {
    $.ajax({
        type: "post",
        url: "<?php echo site_url() . "/" . $this->config->item('ac_base_category') ?>get_main_category_ajax",
        data: {
...
function insert_main_category() {
    ...
    if (message != "") {
        ...
        $.ajax({
            type: "post",
            url: "<?php echo site_url() . "/" . $this->config->item('ac_base_category') ?>insert_money_category_main_ajax",
            data: {
...
function update_main_category() {
    ...
    if (message != "") {
        ...
        $.ajax({
            type: "post",
            url: "<?php echo site_url() . "/" . $this->config->item('ac_base_category') ?>update_money_category_main_ajax",
            data: {
                'new_name': new_name,
...
function delete_main_category(bc_id) {
    ...
    $.ajax({
        type: "post",
        url: "<?php echo site_url() . "/" . $this->config->item('ac_base_category') ?>delete_money_category_main_ajax",
        data: {
            'bc_id': bc_id
        },
        dataType: "JSON",
        success: function(data) {
            ...
            if (data["json_data_exist"] == "Y") {
                //แจ้งเตือนหมวดเงินถูกใช้งานอยู่่
                show_warning("รายการนี้ถูกใช้งานอยู่ <br> ไม่สามารถลบได้");
            } //if
            else {
                //แจ้งเตือนลบสำเร็จ
                Swal.fire({
